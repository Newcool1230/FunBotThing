...
function JoinLeave(user){
	var staffam = API.getStaff().length;
...
function chatMe(msg){
		...
		if(data.message.indexOf('!') === 0 || data.message.indexOf('.') === 0){
			...
			if(Funbot.misc.ready || Funbot.admins.indexOf(uid) > -1 || API.getUser(data.uid).role > 1 || API.getUser(uid).role < 2){
				switch(command[0].toLowerCase()){
					case "command":
					case "commands":
						var sentences = ["[NoBot] [" + command[0].toLowerCase() + "] @" + un + ", sorry I'm too lazy to show you my commands","[NoBot] [" + command[0].toLowerCase() + "] @" + un + " Here, I like you: http://pastebin.com/raw.php?i=yHPMbQjT"];
					...
					case "test":
						var sentences = ["[NoBot] [" + command[0].toLowerCase() + "] @" + un + " Stahp I'm busy","[NoBot] [" + command[0].toLowerCase() + "] @" + un + " Test succeeded :white_check_mark:"];
						var a = Math.floor(Math.random()*sentences.length);
						API.sendChat(sentences[a]);
						API.moderateDeleteChat(msgid);
						break;
					...
					case "ping":
						API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + un + ", pong!");
						break;

					case "marco":
						API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + un + ", polo!");
						break;
					...
					case "say":
						if(API.getUser(uid).role > 1 || Funbot.admins.indexOf(uid) > -1 || typeof command[1] === "undefined"){
							...
							API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + un + ", this command requires staff members only!");
						}
						API.moderateDeleteChat(msgid);
						break;
					...
					case "pp":
					case "xp":
						API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + un + " - Every minute, the game checks what you did, such as wooting, mehing, and chatting");

					case "rules":
						API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + un + " *yawns* Oke, oke, gimme a sec");
					...
					case "linkin":
					case "link":
						API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + un + ", was that ever a command? Well it's not anymore.");
						API.moderateDeleteChat(msgid);
						break;

					case "props":
						if(typeof command[1] === "undefined"){
							API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @"+ API.getDJ().username +", " + un + " congratulated you for your awesome song");
						}
						API.moderateDeleteChat(msgid);
						break;

					case "version":
						API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + un + " " + Funbot.misc.version);
						API.moderateDeleteChat(msgid);
					...
					case "source":
						API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + un + " DJ - ɴᴇᴏɴ - TFL originally wrote me at github, available here: http://goo.gl/iLRyWJ");
						API.moderateDeleteChat(msgid);
						break;

					case "reload":
						if(API.getUser(uid).role > 1 || Funbot.admins.indexOf(uid) > -1 || API.getUser().username == "Beta Tester"){
							API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] Now reloading script...");
							setTimeout(function(){Funbot.unhook();}, 150);
							setTimeout(function(){Funbot.hook();}, 550);
						}else{
							API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + un + " This command requires bouncer +");
						}
						API.moderateDeleteChat(msgid);
						Funbot.misc.ready = false;
						setTimeout(function(){ Funbot.misc.ready = true; }, Funbot.settings.cooldown * 1000);
					...
					case "die":
						if(API.getUser(uid).role > 1 || Funbot.admins.indexOf(uid) > -1 || API.getUser().username == "Beta Tester"){
							API.moderateDeleteChat(msgid);
							Funbot.unhook();
							setTimeout(function(){API.sendChat('[NoBot] [' + command[0].toLowerCase() + '] Murder is punishable w........ ');}, 700);
					...
					case "whyauto":
					case "whywoot":
						API.sendChat("[NoBot]  [" + command[0].toLowerCase() + "] @" + un + " If you are in this room, it means you'll most probably like the song that's playing. Therefore, you'll woot most of the songs. Autowoot does it for you.");
						API.moderateDeleteChat(msgid);
						Funbot.misc.ready = false;
						setTimeout(function(){ Funbot.misc.ready = true; }, Funbot.settings.cooldown * 1000);
						break;
					...
					case "help":
						...
						var sentences = ["[NoBot] [" + command[0].toLowerCase() + "] @" + un + " Kill it with fire.","[NoBot] [" + command[0].toLowerCase() + "] @" + un + " Someone, call 911, this dude needs help over here","[NoBot] [" + command[0].toLowerCase() + "] @" + un + " Do I really need to work on a " + n + "??","[NoBot] [" + command[0].toLowerCase() + "] @" + un + " I think it's time to call a BA.","[NoBot] [" + command[0].toLowerCase() + "] Ghaaa, are you sure you need me?"];
						var a = Math.floor(Math.random()*sentences.length);
						API.sendChat(sentences[a]);
						API.moderateDeleteChat(msgid);
						Funbot.misc.ready = false;
					...
					case "creator":
						if(Funbot.admins.indexOf(uid) !== -1 || API.getUser(uid).role < 2){
							API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + un + " This bot was originally created by: ๖ۣۜĐل - ɴᴇᴏɴ - TFL, and it's copyrighted! Edited by T98 for educational purposes.");
						}
						API.moderateDeleteChat(msgid);
						Funbot.misc.ready = false;
						setTimeout(function(){ Funbot.misc.ready = true; }, Funbot.settings.cooldown * 1000);
					...
					case "dclookup":
					case "dc":
						var sentences = ["[NoBot] [" + command[0].toLowerCase() + "] @" + un + " Oh come on the WaitList isn't that big","[NoBot] [" + command[0].toLowerCase() + "] @" + un + " Sorry, too lazy to move anyone right now"];
						var a = Math.floor(Math.random()*sentences.length);
						API.sendChat(sentences[a]);
						API.moderateDeleteChat(msgid);
						Funbot.misc.ready = false;
					 ...
					 case "8ball":
						if(typeof command[1] == "undefined"){
							...
							switch(randomSentence){
								case 0:
									API.sendChat("@" + un + " " + Funbot.misc.ball[randomBall]);
									break;
								case 1:
									API.sendChat("@" + un + " " + Funbot.misc.ball[randomBall]);
									break;
					...
					case "flipcoin":
						if(typeof command[1] == "undefined"){
							...
							switch(randomSentence){
								case 0:
									API.sendChat("@" + un + " " + Funbot.misc.ht[randomHt]);
									break;
								case 1:
									API.sendChat("@" + un + " " + Funbot.misc.ht[randomHt]);
									break;
					...
					case "punish":
						if(typeof command[1] == "@"){
							...
							switch(randomSentence){
								case 0:
								case 1:
									API.sendChat("/me stares at " + crowd[randomUser].username + " in a creepy way");
									break;
								case 2:
								case 3:
						...
						case "dogfact":
							if(typeof command[1] == "undefined"){
								...
								switch(randomSentence){
									case 0:
										API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + un + " " + Funbot.misc.dogfact[randomDogfact]);
										break;
									case 1:
										API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + un + " " + Funbot.misc.dogfact[randomDogfact]);
										break;
								...
								switch(randomSentence){
									case 0:
										API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + command[1] + " " + Funbot.misc.dogfact[randomdogfact]);
										break;
									case 1:
										API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + command[1] + " " + Funbot.misc.dogfact[randomDogfact]);
										break;
						...
						case "catfact":
							if(typeof command[1] == "undefined"){
								...
								switch(randomSentence){
									case 0:
										API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + un + " " + Funbot.misc.catfact[randomCatfact]);
										break;
									case 1:
										API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + un + " " + Funbot.misc.catfact[randomCatfact]);
										break;
								...
								if(command[1].indexOf("@") === 0) command[1] = command[1].substring(1);
									...
									switch(randomSentence){
										case 0:
											API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + command[1] + " " + Funbot.misc.catfact[randomCatfact]);
											break;
										case 1:
											API.sendChat("[NoBot] [" + command[0].toLowerCase() + "] @" + command[1] + " " + Funbot.misc.catfact[randomCatfact]);
											break;
	...
	API.on(API.CHAT, function(data){
		...
		if(msg.indexOf("i got to go") !== -1 || msg.indexOf("igtg") !== -1 || msg.indexOf("gtg") !== -1 || msg.indexOf("be back") !== -1 || msg.indexOf("going off") !== -1 || msg.indexOf("off to") !== -1 || msg.indexOf("have to go") !== -1 || msg.indexOf("bye bot") !== -1 || msg.indexOf("bot bye") !== -1){
			var AfkMsg = ["[NoBot] @" + un + " See ya man!","[NoBot] @" + un + " Awww, See ya babe.","[NoBot] @" + un + " Glad you came by thanks! :kissing_heart:","[NoBot] @" + un + " Thanks for coming. Hope to see you soon! :blue_heart:"];
